(function(window, angular) {
  angular
        .module('articleController', ['store-factories', 'model-factories'])
        .controller('ArticleListController', ArticleListController)
        .controller('ArticleViewController', ArticleViewController);

  function ArticleListController($scope, rest, $window, usSpinnerService, modelFactory, $routeParams, $anchorScroll) {
      ...
      if(!angular.isUndefined($routeParams.category)) {
          rest().get({
            ...
            // console.log(resp);
            if(!!resp) {
              vm.category = resp[0].name;
              vm.paginate.conditions = 'category=' + resp[0]._id;
              loadModel();
            }
          });
      } else {
        vm.category = 'Institucional';
        loadModel();
  ...
  function ArticleViewController($scope, $routeParams, rest, $location, usSpinnerService, modelFactory, $sce) {
      usSpinnerService.spin('spinner');
      //Variables
      var vm = this;
      vm.module = 'articles';
      ...
      function loadModel() {
          rest().get({
            type: vm.module,
            params: 'slug=' + $routeParams.slug + '&populate=front,images,category'
          }, function(resp) {
            // console.log(resp[0]);
            if(!!resp[0]) {
              vm.info = resp[0];
              ...
              if (!!resp[0].date) {
                  vm.info.date = vm.info.date ? moment(vm.info.date).utc().toISOString().slice(0, 10) : null;
              }
              //
              // rest().findOne({
              //   type: 'categories',
